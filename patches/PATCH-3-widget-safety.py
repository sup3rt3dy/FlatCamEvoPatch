...
def apply_patch():
    ...
    # Find FCCheckBox class and its get_value method
    search_pattern = '''class FCCheckBox(QtWidgets.QCheckBox):
    """
    Custom QCheckBox
    """
    ...
    replacement_pattern = '''class FCCheckBox(QtWidgets.QCheckBox):
    """
    Custom QCheckBox
    """
    ...
    def get_value(self):
        """
        Gets the state of the checkbox.
        Safely handles widget deletion during threading operations.
    ...
    if search_pattern not in content:
        ...
        # Try simpler search
        if "class FCCheckBox" in content and "def get_value(self):" in content:
            print("  ✓ Found FCCheckBox class")
            # Find the method and patch it more flexibly
            import re
            
            pattern = r'(class FCCheckBox\(QtWidgets\.QCheckBox\):.*?def get_value\(self\):.*?""".*?""")\s+return self\.isChecked\(\)'
        ...
        try:
            ...
            if re.search(pattern, content, re.DOTALL):
                ...
                print("  ✓ Patched using flexible regex pattern")
            else:
                print("  ✗ Could not match flexible pattern either")
                return False
        else:
            return False
    else:
